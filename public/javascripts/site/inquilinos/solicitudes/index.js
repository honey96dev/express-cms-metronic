...
Solicitudes.prototype.init = function () {
    const self = this;
    self.baseUrl = $('#baseUrl').val();

    $.ajax({
        url: self.baseUrl + 'solicitudes/list',
        method: 'GET',
        dataType: 'json',
...
Solicitudes.prototype.generateList = function(items) {
    ...
    for (let item of items) {
        ...
        if (item.photo.length > 0) {
            card = '<div class="col-xl-3 col-lg-4 col-md-4 col-sm-6 col-xm-12 card-wrap" data-id="' + item.application_id + '"><div class="kt-portlet kt-portlet--height-fluid my-card"><div class="kt-portlet__body list-card-body"><div class="list-card-img" style="background-image: url(/uploads/photo/' + item.photo + ')"><div class="list-card-trash"><button class="list-card-trash-button btn btn-warning btn-elevate btn-circle btn-icon list-card-trash-icon  btn-orange"><i class="flaticon-delete list-card-trash-icon"></i></button></div></div><div class="list-card-name-badge">' + item.name + '</div><div class="list-card-address">' + item.address + '</div><div class="list-card-property-sec row"><div class="col-4 list-card-property"><li class="fa fa-expand-arrows-alt"></li>' + item.surface + 'm2</div><div class="col-4 list-card-property"><li class="fa fa-bed"></li>' + item.rooms + 'rooms</div><div class="col-4 list-card-property"><li class="fa fa-bath"></li>' + item.baths + 'baths</div></div></div></div></div>';
        } else {
            ...
            card = '<div class="col-xl-3 col-lg-4 col-md-4 col-sm-6 col-xm-12 card-wrap" data-id="' + item.application_id + '"><div class="kt-portlet kt-portlet--height-fluid my-card "><div class="kt-portlet__body list-card-body"><div class="list-card-solid"><div class="list-card-trash"><button class="list-card-trash-button btn btn-warning btn-elevate btn-circle btn-icon list-card-trash-icon  btn-orange"><i class="flaticon-delete list-card-trash-icon"></i></button></div><div class="list-card-name">' + item.name + '</div></div><div class="list-card-address">' + item.address + '</div><div class="list-card-property-sec row"><div class="col-4 list-card-property"><li class="fa fa-expand-arrows-alt"></li>' + item.surface + 'm2</div><div class="col-4 list-card-property"><li class="fa fa-bed"></li>' + item.rooms + 'rooms</div><div class="col-4 list-card-property"><li class="fa fa-bath"></li>' + item.baths + 'baths</div></div></div></div></div>';
        }
        html += card;
    }
...
Solicitudes.prototype.showErrorMsg = function (container, type, msg) {
...
$(document).on("click", ".card-wrap", function (e) {
    ...
    if(target.hasClass("list-card-trash-button") || target.hasClass("list-card-trash-icon")) {
        ...
        document.location = instance.baseUrl + 'application?app_id=' + id;
    }
});
