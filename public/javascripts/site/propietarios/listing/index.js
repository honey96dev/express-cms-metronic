...
Listing.prototype.generateList = function(items) {
    ...
    for (let item of items) {
        ...
        if (item.photo.length > 0) {
            card = '<div class="col-xl-3 col-lg-4 col-md-4 col-sm-6 col-xm-12" onclick="instance.editItem(' + item.id + ')"><div class="kt-portlet kt-portlet--height-fluid my-card "><div class="kt-portlet__body list-card-body"><div class="list-card-img" style="background-image: url(/uploads/photo/' + item.photo + ')"><div class="list-card-trash"><button class="list-card-trash-icon">asdf</button></div></div><div class="list-card-name-badge">photo' + item.name + '</div><div class="list-card-address">' + item.address + '</div><div class="list-card-property-sec row"><div class="col-4 list-card-property"><li class="fa fa-expand-arrows-alt"></li>' + item.surface + 'm2</div><div class="col-4 list-card-property"><li class="fa fa-bed"></li>' + item.rooms + 'rooms</div><div class="col-4 list-card-property"><li class="fa fa-bath"></li>' + item.baths + 'baths</div></div></div></div></div>';
        } else {
...
Listing.prototype.editItem = function(id) {
    const self = this;
    document.location = self.baseUrl + 'anuncios/nueva?id=' + id;
};
