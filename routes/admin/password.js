...
const resetPasswordGetProc = (req, res, next) => {
    ...
    let sql = sprintfJs.sprintf("SELECT T.*, U.name FROM `tokens` T JOIN `%s` U ON U.email = T.email WHERE BINARY T.token = '%s';", config.dbTblName.admins, token);
...
const resetPasswordPostProc = (req, res, next) => {
    ...
    dbConn.query(sql, null, (error, results, fields) => {
        ...
        if (count === 0) {
            ...
            console.log('verify', timestamp, expire);
            if (timestamp < expire) {
                sql = sprintfJs.sprintf("UPDATE `admins` SET `password` = '%s' WHERE BINARY `email` = '%s';", hash, email);
                dbConn.query(sql, undefined, (error, results, fields) => {
                    if (error) {
                        res.status(200).send({
                            result: 'error',
